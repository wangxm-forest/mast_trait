...
\begin{itemize}
\item Angiosperm (124 total ; 79\% (98/124) with masting data; Masting and non-masting split almost equally (52\%(51/98), 48\%(47/98)); Gymnosperm (64 total; 95\% (61/64) with masting data; almost all species are masting (89\%(54/61), 11\%(7/61)))
\item Masting more common for gymnosperm
\end{itemize}
...
Dispersal mode & Biotic vs abiotic & 0.55 & 0.53 & 0.30 & 0.06 &  93 \\ 
  ...
  Drought tolerance & Moderate vs high drought tolerance & -0.13 & 0.46 & 0.79 & 0.11 &  79 \\ 
   \hline
\end{tabular}
\caption{Phylogenetic generalized linear model results for angiosperm. Estimate represents the log-odds associated with each predictor. SE is the standard error, and P indicates statistical significance, with P < 0.05 considered significant and marked with an asterisk (*) in the table. “Phylo $\alpha$” represents the phylogenetic signal estimated for each trait using phyloglm, lower values indicate a stronger phylogenetic signal for that trait. N indicates the number of observations available for each trait.} 
\label{tab:regressiongym} 
...
\begin{itemize}
\item Only seed weight is related to masting with having bigger seeds will increase the chance of being a masting species Figure (\ref{fig:rawangio}), At the mean log seed weight (4.54), the predicted probability of masting was 0.43. (\ref{tab:regressionangio}). At the 75th percentile log seed mass (7.5103), the predicted probability of masting was 0.6. Only within angiosperm we found this significant effect (p-value = 0.02) of seed weight.
\end{itemize}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{../output/figures/weightTreeConcept1.jpg}
