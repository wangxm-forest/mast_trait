...
\caption{Predicted likelihood of masting as a function of seed mass for all angiosperm species (solid red line) and the biotic-dispersed subgroup (dashed green line), estimated using phylogenetic logistic regression (phyloglm). Individual species observations are plotted as binary data points at 0 (non-masting) and 1 (masting).}
...
\begin{enumerate}
\item Only seed weight is related to masting with having bigger seeds will increase the chance of being a masting species Figure (\ref{fig:rawangio}). A tripling of seed weight (from a sample median baseline of 90.9g to 272.7g) is predicted to increase the likelihood of masting from 0.45 to 0.51 Figure (\ref{fig:seedweight}). And this increase is stronger in biotic-dispersed seed group Table (\ref{tab:regressionseedweight}), Figure (\ref{fig:seedweight}). Only within angiosperm we found this significant effect (p-value = 0.02) of seed weight.
\end{enumerate}
