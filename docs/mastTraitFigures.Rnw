...
<<load-plots,eval=FALSE,echo=FALSE>>=
...
\section{Results}
% \subsection{Gathered data} %emw16Jun -- the results is your subsection, can you please use enumerate instead of subsection throughout? I show how here for this first one. 
\begin{enumerate}
\item Gathered data
\begin{enumerate}
\item Structure of data from Silvics
\begin{enumerate}
\item Angiosperm (124 total; 79\% (98/124) with masting data; Masting and non-masting split almost equally (52\%(51/98)); Gymnosperm (64 total; 95\% (61/64) with masting data; almost all species are masting (89\%(54/61)) %emw16Jun: These #s need to be done with \Sexpr *and* don't repeat #s here -- so you say there are 124 total and then 98/124 -- you just need to say: 79% (98/124) masting ... 
\end{enumerate}
...
<<Table phyloglmangio,results='tex',echo = FALSE>>=
library(xtable)
