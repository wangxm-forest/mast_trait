...
\item Angiosperm \Sexpr{gsub("%", "\\\\%", angio_sum)} with masting data; Strong asting and weak masting split almost equally \Sexpr{gsub("%", "\\\\%", angio_mast)}; Gymnosperm \Sexpr{gsub("%", "\\\\%", gymno_sum)} with masting data; almost all species are strong masting \Sexpr{gsub("%", "\\\\%", gymno_mast)}
\end{enumerate}
...
of being strong masting species. Gymnosperm are all wind pollinated, and gymnosperm has a greater portion of strong masting species (89\% (54/61)).
\end{enumerate}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{../output/figures/weightTreeConceptual3.jpg}
