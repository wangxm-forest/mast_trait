...
%emw16Jun: Nice work! This is feeling closer. I think now we should indicate what is supp and what will be main text, so can you indicate here what will be in the supp for tables and figures? Those should all come AFTER the main text figures, and eventually should go in a supp file. Also we should start moving towards captions we could send to a journal. See also my comments throughout with: %emw16Jun
\section{Results}
\begin{enumerate}
\item Gathered data
\begin{enumerate}
...
@
\item Angiosperm \Sexpr{gsub("%", "\\\\%", angio_sum)} with masting data; Masting and non-masting split almost equally \Sexpr{gsub("%", "\\\\%", angio_mast)}; Gymnosperm (6\Sexpr{gsub("%", "\\\\%", gymno_sum)} with masting data; almost all species are masting \Sexpr{gsub("%", "\\\\%", gymno_mast)}
\end{enumerate}
...
%emw16Jun: I know it's more work but I want the same size #s ABOVE each violin plot -- just like the boxplots. Also make the legends masting and non-masting (not yes/no). 
\newgeometry{margin=1cm}
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth, height=0.8\textheight]{../output/figures/rawDataWithStatsAngio.pdf} %emw16Jun: I edited the caption of this figure to be something closer to publishable, can you make similar changes to the others, please? Also I think it is predator satiation, right? Not predation satiation. 
\caption{How plant traits related to different hypotheses differ in masting and non-masting species: We show data for traits related to the predator satiation hypothesis (a-e), pollination coupling hypothesis (f-g) and the resource matching hypothesis (h-i) as either violin plots (1-c, i) or proportional histograms (d-h) separated by species categorized as masting (red) or non-masting (green). We show model summaries only where significant (P-value < 0.5) and also report the model sample size (N), regression coefficient (Estimate), and phylogenetic signal parameter $\alpha$. Full model results are show in supp table XX.}
...
Seed weight (log) & logSeedWeight* & Numeric & 0.21 & 0.09 & 0.02 & 8.54 & 0.08 &  91 \\ 
  ...
  Drought tolerance & Low vs high drought tolerance & Categorical & 0.14 & 0.47 & 0.76 & 12.57 & 0.06 &  79 \\ 
  Drought tolerance & Moderate vs high drought tolerance & Categorical & -0.13 & 0.46 & 0.78 & 12.57 & 0.06 &  79 \\ 
  Leaf longevity (years) & Leaf longevity (years) & Numeric & 0.26 & 0.95 & 0.78 & 0.90 & 0.77 &  16 \\ 
   \hline
\end{tabular}
}
 %emw16Jun: I edited the caption of this table to be something closer to publishable, can you make similar changes to the others, please? Also please move the N column to after or before Type (it's more information about the data than the model estimates, it should go earlier).  
  %emw16Jun: Are you sure values less than 1 half time represent "relatively weak" signal -- that's everything, but leaf longeivity is close to 1 ... so what do I make of that? 
 ...
 %  \caption{Phylogenetic generalized linear model results for angiosperm. Each trait was evaluated in a separate model structured as Masting$\sim$Trait, where masting(Yes/No) is the binary response variable.  For categorical traits, the estimate represents the difference in log-odds between a given level and its designated baseline reference level. For numeric traits, the estimate represents the change in log-odds per unit increase of the predictor. SE is the standard error, and P indicates statistical significance, with P < 0.05 considered significant and marked with an asterisk (*) in the table. “Phylo $\alpha$” represents the phylogenetic signal in the residuals estimated for each trait using phyloglm, we calculated half-time (log(2)/$\alpha$) to compare with the tree height, which represents the time required for the influence of ancestral states to decrease by half. The tree we used is ultrametric with a standardized height of 1. If half-time is greater than tree height, it indicates strong phylogenetic signal in the residuals; if half-time is smaller than tree height, it indicates relatively weak phylogenetic signal in the residuals. N indicates the number of observations available for each trait.} 
...
\caption{Phylogenetic generalized linear model results for gymnosperm. Each trait was evaluated in a separate model structured as Masting$\sim$Trait, where masting(Yes/No) is the binary response variable.  For categorical traits, the estimate represents the difference in log-odds between a given level and its designated baseline reference level. For numeric traits, the estimate represents the change in log-odds per unit increase of the predictor. SE is the standard error, and P indicates statistical significance, with P < 0.05 considered significant and marked with an asterisk (*) in the table. “Phylo $\alpha$” represents the phylogenetic signal in the residuals estimated for each trait using phyloglm, we calculated half-time (log(2)/$\alpha$) to compare with the tree height, which represents the time required for the influence of ancestral states to decrease by half. The tree we used is ultrametric with a standardized height of 1. If half-time is greater than tree height, it indicates strong phylogenetic signal in the residuals; if half-time is smaller than tree height, it indicates relatively weak phylogenetic signal in the residuals. N indicates the number of observations available for each trait.} 
...
\begin{tabular}{llrrrrrrr}
  \hline
Group & Trait & Predictor & Estimate & SE & P & Phylo $\alpha$ & Half-time & N \\ 
  \hline
Biotic dispersed & Seed weight (log) & logSeedWeight & 0.40 & 0.15 & 0.01 & 54.57 & 0.01 &  53 \\ 
Abiotic dispersed & Seed weight (log) & logSeedWeight & 0.12 & 0.12 & 0.36 & 50.33 & 0.01 &  36 \\ 
   \hline
\end{tabular}
}
\caption{Phylogenetic Generalized Linear Model Results for only seed weight for biotic-dispersed subgroup and abiotic-dispersed subgroup} 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{../output/figures/modelFitSeedWeight.pdf}  %emw16Jun: Nice, can you color code the dots in light green if they are biotic dispersed? 
\caption{Predicted likelihood of masting as a function of seed weight for all angiosperm species (solid red line) and the biotic-dispersed subgroup (dashed green line), estimated using phylogenetic logistic regression. Individual species observations are plotted as binary data points at 0 (non-masting) and 1 (masting).}
...
\begin{enumerate} %emw16Jun: 
\item Only seed weight is related to masting with larger seeds increasing the probability of being a masting species (Figure (\ref{fig:rawangio})). For angiosperms only, tripling of seed weight (from a median seed weight of 90.9g to 272.7g) is predicted to increase the likelihood of masting from 0.45 to 0.51 (Figure (\ref{fig:seedweight})). %emw16Jun:  much better!
And this increase is stronger in biotic-dispersed seed group (Table (\ref{tab:regressionseedweight}); Figure (\ref{fig:seedweight})). %emw16Jun: if you're saying it's stronger than also give here the predicted increase in likelihood of masting from 0.XX to 0.YY for this group. 
% Only within angiosperm we found this significant effect (p-value = 0.02) of seed weight. %emw16Jun: if it's not significant then you do not report it. 
\end{enumerate}
\item Pollination satiation: wind pollination for angiosperm
\begin{enumerate}
\item we found wind pollination is related to masting for angiosperm species, wind pollinated species have a significantly higher probability (59\%) %emw16Jun:  what do I compare this to? 50% or 0%? I think you need another number ... 
of being strong masting species. Gymnosperm are all wind pollinated, and gymnosperm has a greater portion of masting species (89\% (54/61)).
\end{enumerate}
...
\begin{enumerate} 
\item We don't see masting aligned well along any axis in the NMDS analysis. %emw16Jun: I don't like to use contractions (e.g., don't) in scientific writing. You could just masting is not well aligned .... 
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.9\textheight,keepaspectratio]{../output/figures/weightTreeConceptual2.jpg}
