...
\item Masting more common for gymnosperms
\end{itemize}
\item Number of traits for different hypotheses
\begin{itemize}
\item For angiosperms, we have data for:
\begin{itemize}
...
\end{itemize}
\item For gymnosperms, we have the same data as for angiosperms, but we only have oil content for masting species and all the gymnosperm species are wind pollinated.
\end{itemize}
\end{enumerate}
...
Dispersal mode & Biotic vs abiotic & 0.55 & 0.53 & 0.30 & 0.06 &  93 \\ 
  ...
  Oil content (\%) & Oil content (\%) & 0.03 & 0.03 & 0.22 & 0.14 &  19 \\ 
  ...
  Drought tolerance & Moderate vs high drought tolerance & -0.13 & 0.46 & 0.79 & 0.11 &  79 \\ 
   \hline
\end{tabular}
\caption{Phylogenetic generalized linear model results for angiosperms. Estimate represents the log-odds associated with each predictor. SE is the standard error, and P indicates statistical significance, with P < 0.05 considered significant and marked with an asterisk (*) in the table. “Phylo $\alpha$” represents the phylogenetic correlation parameter estimated for each trait using phyloglm, lower values indicate a stronger phylogenetic signal for that trait. N indicates the number of observations available for each trait.} 
...
Seed dispersal & Biotic vs abiotic & -0.83 & 0.81 & 0.30 & 0.01 &  60 \\ 
  ...
  Seed size (log) & logSeedSize & -0.37 & 0.93 & 0.69 & 0.41 &  51 \\ 
  ...
  Drought tolerance & Moderate vs high drought tolerance & -1.56 & 1.06 & 0.14 & 0.43 &  52 \\ 
   \hline
\end{tabular}
\caption{Phylogenetic generalized linear model results for gymnosperms. Estimate represents the log-odds associated with each predictor. SE is the standard error, and P indicates statistical significance, with P < 0.05 considered significant and marked with an asterisk (*) in the table. “Phylo $\alpha$” represents the phylogenetic signal estimated for each trait using phyloglm, lower values indicate a stronger phylogenetic signal for that trait. N indicates the number of observations available for each trait.} 
\label{tab:regressiongym}
\end{table}
\end{landscape}
\newpage
...
\begin{itemize}
\item Predation satiation: seed weight only with a small effect and only for angiosperms
\begin{itemize}
\item Only seed weight is related to masting with having bigger seeds will increase the chance of being a masting species Figure (\ref{fig:rawangio}). An increase in seed weight in the log-scale by about 3 would increase the predicted probability of masting from 0.43 to 0.6. Only within angiosperms we found this significant effect (p-value = 0.02, Table \ref{regressionangio}) of seed weight.
\end{itemize}
\item Pollination satiation: wind pollination for angiosperms
\begin{itemize}
\item we found wind pollination is related to masting for angiosperm species, wind pollinated species have a significantly higher probability (59\%) of being strong masting species compared to animal pollinated species. Gymnosperms are all wind pollinated, and gymnosperms have a greater portion of masting species (89\% (54/61)).
\end{itemize}
...
\begin{itemize}
\item Masting shows a moderate phylogenetic signal in both gymnosperms (estimated D = 0.51) and angiosperms (estimated D = 0.58).
\item Seed weight has strong phylogenetic signal for both angiosperms ($\lambda$ = 0.95) and gymnosperms ($\lambda$ = 0.97). We can see the similarity in certain group in Figure \ref{fig:coneptangio} it's relatively reliable estimate. Dispersal mode, leaf longevity pollination mode and reproductive type also showed high phylogenetic signal, but leaf longevity data is nested in certain clades, and the other traits are categorical traits.
