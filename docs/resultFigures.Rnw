...
\item Angiosperm \Sexpr{gsub("%", "\\\\%", angio_sum)} with masting data; Strong masting and weak masting split almost equally \Sexpr{gsub("%", "\\\\%", angio_mast)}; Gymnosperm \Sexpr{gsub("%", "\\\\%", gymno_sum)} with masting data; almost all species are strong masting \Sexpr{gsub("%", "\\\\%", gymno_mast)}
\end{enumerate}
