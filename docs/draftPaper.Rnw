...
\subsection{Definition of masting} %emw16Jun: can you please switch to enumerate instead of subsection? See my example in the methods. 
\begin{enumerate}
...
\begin{enumerate} %emw16Jun: do the below go in a certain order? If so I might add that ... First,  Pollination success (Pollination coupling) ... next growing seeds, Resources availability (Resource Matching)
...
\item Next step may be clear connection between traits and hypotheses and potential evolutionary drivers %emw16Jun: I am pretty negative so I always try when I can to frame 'you didn't do this!' to ''ah, maybe now we need to ...' 
\end{enumerate}
...
\item This framework is especially useful when considering whether evolutionary drivers are more similar within closely related species (clades) %emw16Jun: when writing for ecologists say 'clades' sparingly, as it confuses them. 
\end{enumerate}
\subsection{We try to do this using data from one specific geographical region}
\begin{enumerate}
...
\end{enumerate}
\subsection{Phylogenetic signal for traits and masting}
\begin{enumerate}
...
\end{enumerate}
\subsection{Deep evolutionary split between angiosperm and gymnosperm} %emw25May2026 -- at some point we may want to make the point that basically we think these relationships may appear at different scales -- either below the population level or while controlling for deep evolutionary time... 
\begin{enumerate}
...
\item Test if masting was gradually diluted in angiosperm as they transitioned to biotic pollination and faster life histories  %emw16Jun: Cool!
\end{enumerate}
\end{enumerate}
\end{enumerate}
