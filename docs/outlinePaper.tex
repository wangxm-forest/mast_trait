...
\usepackage{Sweave}
\begin{document}
\input{outlinePaper-concordance}
...
\begin{enumerate}
\item Mechanisms are not mutually exclusive, but some might act stronger than the other for certain species
...
\item All these studies use CV to quantify masting, explain the pros and cons of CV.
\item If evolutionary associations are sufficiently strong, we should be able to detect it even under a coarse binary classification
\item Recent papers also focus on resource perspective using leaf and shoot traits
\end{enumerate}
\item Existing studies mainly focus on ecological mechanisms.
\item Next step may be clear connection between traits and hypotheses and potential evolutionary drivers for certain species
\end{enumerate}
...
\item Includes both strong masting and weak masting species
\item We try to understand this on species level, so we are using a binary trait here instead of CV
\end{enumerate}
\end{enumerate}
\end{enumerate}
...
\item These act at the beginning and end of the reproductive cycle, two critical stages for successful reproduction and gene transmission
...
\item At the same time, they have to make sure the seed survived can established
\item Selection may favor coordinated strategies that increase seed survival across the whole recruitment process (\cite{bonal2007satiation, moles2004seedling})
...
\begin{enumerate}
\item Many traits have strong phylogenetic signal, it is hard to tease out traits from masting, which limits inference
\item The lit on this mostly talks about adaptive significance, we need to understand its broader macroevolutionary patterns
\item Data is strongly nested in phylogeny, and data is sparse.
\end{enumerate}
\item Deep evolutionary split between angiosperm and gymnosperm 
\begin{enumerate}
...
\item Importance of testing within clade
\begin{enumerate}
...
\item This requires more data collection on species-level and population-level within certain genus groups, for example: (\textit{Quercus.})
...
\item Data scraping
  ...
  \begin{enumerate}
  \item Definition of strong masting vs. weak masting
